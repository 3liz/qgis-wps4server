...
# for proper Exception debug it should be better to use a try: exception.
# See: soap_testes.testSOAP11Fault

    def setUp(self):
        # Silence sterr otherwise the promopt is flooded with error message
        # from exceptions
        sys.stderr = open('/dev/null', "w")

    def testMaxFileProcess(self):
        """Text exception raise from MaxFileSize (process and cfg file)"""
        # Calling complexRaster process that has datainput with
        # maxfilezie=2.0megas below 3mega of pywps.cfg

        self._setFromEnv()

        mypywps = pywps.Pywps(pywps.METHOD_GET)
        inputs = mypywps.parseRequest(
            "service=wps&request=execute&version=1.0.0&identifier=complexRaster&datainputs=[indata=%s]" % self.tiffurl)
        ...
        self.assertEqual(exceptionDOM[0].getAttribute(
            "exceptionCode"), "FileSizeExceeded")
        ...
        inputs = mypywps.parseRequest(
            "service=wps&request=execute&version=1.0.0&identifier=complexprocess&datainputs=[rasterin=%s]" % (urllib.quote(self.tiffurl)))
        mypywps.performRequest()
        xmldom = minidom.parseString(mypywps.response)
        exceptionDOM = xmldom.getElementsByTagNameNS(self.owsns, "Exception")
    ...
    def testQuoteChar(self):
        ...
        self.assertEquals(
            'Failed to execute WPS process [proc_name]: &lt;&lt;to-be-escaped&gt;&gt;', exception_text)
    ...
    def _setFromEnv(self):
        ...
        os.environ["PYWPS_CFG"] = os.path.join(
            pywpsPath, "pywps", "default.cfg")
